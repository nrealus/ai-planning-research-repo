...
def add_constraint(
    ...
    constr_elementary_expr = _transform_and_prepare_constraint_expression_into_elementary_form(constraint_expression, solver)
    scope_lit = _get_or_make_new_scope_literal_from_conjunctive_scope_literals(conjunctive_scope_literals, solver)
    ...
    tauto = _get_tautology_of_scope(scope_lit, solver)
    _bind_elementary_expression(constr_elementary_expr, tauto, solver)
...
def _transform_or(literals: Tuple[Lit,...]) -> ConstraintElementaryExpression.AnyExpr:
...
def _transform_and_prepare_eq(
    ...
    if atom1.var == atom2.var:
        ...
        imply_1_2 = _reify_elementary_expression(_transform_or((lit1.negation(), lit2)), solver)
        imply_2_1 = _reify_elementary_expression(_transform_or((lit2.negation(), lit1)), solver)
        return _transform_or((imply_1_2.negation(), imply_2_1.negation()))

    elif isinstance(atom1, ConstraintExpressionAtoms.Int) and isinstance(atom2, ConstraintExpressionAtoms.Int):
        leq_1_2 = _reify_elementary_expression(_transform_leq(atom1, atom2), solver)
        leq_2_1 = _reify_elementary_expression(_transform_leq(atom2, atom1), solver)
        return _transform_or((leq_1_2.negation(), leq_2_1.negation()))
...
def _get_conjunctive_scope_of_elementary_expr(
...
def _get_or_make_new_scope_literal_from_conjunctive_scope_literals(
    ...
    # (which is equivalent to l <=> l1 & l2 & ... & ln)
        ...
        _bind_elementary_expression(
            _transform_or(tuple(lits)),
            _get_tautology_of_scope(_get_or_make_new_scope_literal_from_conjunctive_scope_literals((), solver), solver), solver)
...
def _reify_elementary_expression(
    constr_elementary_expr: ConstraintElementaryExpression.AnyExpr,
    solver: Solver,
) -> Lit:
    """
    ...
    scope_lit = _get_or_make_new_scope_literal_from_conjunctive_scope_literals(
        _flatten_conjunctive_scope_into_conjunctive_scope_literals(
            _get_conjunctive_scope_of_elementary_expr(constr_elementary_expr, solver),
            False, solver), solver)
...
def _bind_elementary_expression(
    constr_elementary_expr: ConstraintElementaryExpression.AnyExpr,
    lit: Lit,
    solver: Solver,
) -> None:
    ...
    expr_scope_literal = _get_or_make_new_scope_literal_from_conjunctive_scope_literals(
        _flatten_conjunctive_scope_into_conjunctive_scope_literals(
            _get_conjunctive_scope_of_elementary_expr(constr_elementary_expr, solver),
            False, solver), solver)
    ...
    # another reification literal)
        ...
        reification_literal = _reify_elementary_expression(constr_elementary_expr, solver)
        if lit != reification_literal:
            solver.constraints.append(
                (ConstraintElementaryExpression.LitExpr(reification_literal), lit))
...
def _transform_and_prepare_constraint_expression_into_elementary_form(
    ...
    if isinstance(constr_expr, ConstraintExpression.Leq):
        return _transform_leq(constr_expr.left_atom, constr_expr.right_atom)

    elif isinstance(constr_expr, ConstraintExpression.Lt):
        return _transform_leq(
        ...
        return _transform_leq(constr_expr.right_atom, constr_expr.left_atom)
        ...
        return _transform_and_prepare_eq(constr_expr.atom1, constr_expr.atom2, solver)

    elif isinstance(constr_expr, ConstraintExpression.Neq):
        return _transform_and_prepare_eq(constr_expr.atom1, constr_expr.atom2, solver).negation()

    elif isinstance(constr_expr, ConstraintExpression.Or):
        return _transform_or(constr_expr.literals)

    elif isinstance(constr_expr, ConstraintExpression.And):
        return _transform_or(tuple(lit.negation() for lit in constr_expr.literals)).negation()

    elif isinstance(constr_expr, ConstraintExpression.Imply):
        return _transform_or((constr_expr.from_literal.negation(), constr_expr.to_literal))
