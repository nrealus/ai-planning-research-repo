...
class TestElemConstrExpr(unittest.TestCase):
    ...
    def test_from_lits_tighten_and_simplify_or(self):
    ...
    def test_negation(self):
        ...
        self.assertEqual(ElemConstrExpr.from_lit(Lit.leq(A, 1)).negation(),
                         ElemConstrExpr.from_lit(Lit.geq(A, 2)))

        self.assertEqual(ElemConstrExpr.from_max_diff(A, B, 2).negation(),
                         ElemConstrExpr.from_max_diff(B, A, -3))

        self.assertEqual(ElemConstrExpr.from_lits_simplify_or((Lit.leq(A, 1),
                                                               Lit.leq(A, 2),
                                                               Lit.leq(B, 1))).negation(),
